...
export const googleSignIn = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(401).json({
      error: 'Falha na autenticação. Verifique suas credenciais e tente novamente.',
    });
  }
};
  ...
  try {
    ...
    if (!user || !user.password) {
      return res.status(401).json({
        error: 'Credenciais inválidas. Por favor, verifique seu email e senha e tente novamente.',
      });
    }
    ...
    res.status(500).json({
      error: 'Erro durante o login. Tente novamente mais tarde ou entre em contato com o suporte.',
    });
  }
};
...
export const sendVerificationCode = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (email.includes('@gmail.com')) {
      ...
      return res.status(400).json({ error: 'Provedor de e-mail não suportado.' });
    }
...
export const resetPassword = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!user.verificationCode) {
      return res.status(404).json({
        error: 'Codigo de validação incorreto ou não encontrado. Entre em contato com o suporte.',
      });
    }
