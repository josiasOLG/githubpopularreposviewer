...
export const createPlans = async (req: Request, res: Response): Promise<void> => {
  try {
    const plans = [
      {
        name: 'Plano Mensal',
    ...
    console.error('Error creating subscription plans:', error.response?.data || error.message);
    res.status(500).json({ message: 'Error creating subscription plans', error });
  }
};
...
export const listPlans = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ message: 'Error listing subscription plans', error });
  }
};

export const updateBillingInfoController = async (req: Request, res: Response): Promise<any> => {
  const { userId, cardNumber, cardCvc, expMonth, expYear, email, nome } = req.body;

  try {
    const address = await addressRepository.findByIdUser(userId);

    if (!address || address.length === 0 || !address[0].cpf || !address[0].phoneNumber) {
    ...
    const sanitizedTaxId = address[0].cpf.replace(/[.\-]/g, '').replace(/\s+/g, '');
    ...
    const phoneNumber = phoneNumberMatch ? phoneNumberMatch[1].replace(/\D/g, '') : '';
    ...
    const subscriptions = await subscriptionRepository.getAllSubscriptionsByUserId(userId);
    const response = await updateCustomerBillingInfo(subscriptions[0].customer.id, billingInfo);
    ...
    res.status(200).json({ message: 'Billing info updated successfully', response });
  } catch (error: any) {
...
export const getCardTokenController = async (req: Request, res: Response): Promise<any> => {
  try {
    const { userId, cardNumber, cardExpiry, cardCvc, holderName, email, taxId, number } = req.body;
    ...
    const addressPostalCode = address[0].zipCode?.replace(/[.\-]/g, '').replace(/\s+/g, '') ?? '';
    ...
    const customerData = {
      ...
      email: email,
      tax_id: sanitizedtaxId,
      phones: [{ type: 'MOBILE', country: '55', area: phoneArea, number: phoneNumber }],
      address: {
    ...
    return res.status(200).json({ message: 'Token do cartão salvo com sucesso.' });
  } catch (error: any) {
    console.error('Erro ao obter o token do cartão:', error);
    return res.status(500).json({ message: 'Erro ao obter o token do cartão.', error });
  }
};

// Controller to list the card tokens
export const listCardTokens = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    if (!id) {
  ...
  try {
    ...
    if (error.response) {
      ...
      if (errorData && errorData.someSpecificProperty) {
        ...
        console.error('Error creating subscription plans:', JSON.stringify(errorData, null, 2));
      }
    } else {
...
export const getSubscriptionStatus = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.status(500).json({ message: 'Error getting subscription status', error });
  }
};

export const activateSubscriptionStatus = async (req: Request, res: Response): Promise<void> => {
  const { userId } = req.params;
  try {
    const subscription = await SubscriptionModel.findOne({ userId }).exec();
    if (!subscription) {
...
export const cancelSubscriptionStatus = async (req: Request, res: Response): Promise<void> => {
...
export const getCustomersController = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const sanitizedTaxId = address[0].cpf?.replace(/[.\-]/g, '').replace(/\s+/g, '') ?? ''; // error TS2532: Object is possibly 'undefined'.
    ...
    if (!Array.isArray(customers)) {
      res.status(500).json({ message: 'Invalid response format from PagSeguro API' });
      return;
    }

    const customer = customers.find((customer: any) => customer.tax_id === sanitizedTaxId);
...
export const updateCustomerData = async (req: Request, res: Response): Promise<void> => {
  try {
    const { userId, name, email, tax_id, phones, birth_date, address } = req.body;
    ...
    console.error('Error updating customer data:', error.response?.data || error.message);
...
export const renewSubscription = async (req: Request, res: Response): Promise<Response> => {
  try {
    const { userId, cardNumber, expiryMonth, expiryYear, security_code, holderName } = req.body;

    if (!userId || !cardNumber || !expiryMonth || !expiryYear || !security_code) {
    ...
    const sanitizedTaxId = address[0].cpf?.replace(/[.\-]/g, '').replace(/\s+/g, '') ?? ''; // Correção: uso de encadeamento opcional
    ...
    existingSubscription.payment_method = response.payment_method.map((pm: any) => ({
      type: pm.type,
      card: {
        brand: pm.card.brand,
        first_digits: pm.card.first_digits,
